...
import { createFreeCorrection, signedPdfUrl } from '../lib/invoices'
import { formatEUR, parsePrice } from '../lib/money'
import { applyVat, VAT_RATE_PERCENT } from '../lib/tax'
import type { Dealer } from '../types/dealer'
import { useT } from '../i18n'

/** Eine Positionszeile (Strings, damit Tippen flüssig bleibt). */
interface Row {
  description: string
  quantity: string
  unitPrice: string
...
export default function CorrectionNew() {
  ...
  const validRows = rows.filter((r) => r.description.trim() !== '' && qtyOf(r) > 0)
  ...
  async function submit() {
    ...
    setError(null)
    try {
      const ret = await createFreeCorrection({
        dealer_id: dealerId,
      ...
      if (ret.pdf_path) {
        try {
          window.open(await signedPdfUrl(ret.pdf_path), '_blank', 'noopener')
        } catch {
          /* PDF-Öffnen ist optional */
        }
      }
      navigate(`/dealers/${dealerId}`)
    } catch (e) {
      setError(e instanceof Error ? e.message : t('correction.error'))
      setSaving(false)
    }
  }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-medium text-ink">{t('correctionNew.title')}</h1>
        <p className="mt-1 text-sm text-muted">{t('correctionNew.subtitle')}</p>
      </div>
      ...
      <div className="mb-6">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-card text-muted">
              <tr>
                ...
                <th className="w-32 px-3 py-2 text-right font-medium">{t('common.unitPrice')}</th>
        ...
        <p className="mt-1 text-xs text-muted">{t('correctionNew.minusHint')}</p>
      </div>
      ...
      <div className="mb-6 ml-auto max-w-xs rounded-md border-[0.5px] border-line bg-surface px-5 py-4 text-sm">
        ...
        <div className="mt-1 flex justify-between border-t-[0.5px] border-line py-1 pt-2 font-medium">
          <span className="text-ink">{t('correctionNew.gross')}</span>
          <span className="text-ink">− {formatEUR(vat.gross)}</span>
        </div>
      </div>

      <div className="mb-6">
        <label className="mb-1.5 block text-sm text-muted">{t('returns.reason')}</label>
        <textarea
          value={reason}
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {saving ? t('invoiceNew.creating') : t('correctionNew.submit')}
        </button>
      </div>
    </div>
  )
